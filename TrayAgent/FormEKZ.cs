		...
		{ //Повторяет (для гарантии) все действия, которые выполнялись после
		 //ввода Enter в поле txtbINV
			...
			{   //При необходимости переключаемся на нужную БД
				...
				if (rec.Length == 0)
				{ 
					this.txtbBOOK.Text = "Экземпляр не найден!";
					this.btnOK.Hide();
				}
				else
					if (rec.Length > 1)
					{ 
						this.txtbBOOK.Text = "Неоднозначность экземпляра. Найдено записей " + rec.Length;
						this.btnOK.Hide();
					}
					else //Найдена единственная запись.
					//Правда, там еще может оказаться больше одного экземпляра с одинаковым инв. номером или меткой
					{
						...
						this.txtbBOOK.Text = ST.clientEK.FormatRecord("@brief", rec[0]); ;
						//Обязательно проверяем, нет ли двух экземпляров с одинаковым инв. номером или меткой
						//BA5 - если трактовать nh, как инв. номер
						//HA5 - если — как метку
						var BA5 = recEK.Fields.GetField("910")
												.GetField('a', "5")
												.GetField('b', nh)
												.ToArray();
						int Lb = BA5.Length;

						var HA5 = recEK.Fields.GetField("910")
												.GetField('a', "5")
												.GetField('h', nh)
												.ToArray();
						int Lh = HA5.Length;
						...
						if ((Lb + Lh) == 0)
						{
							...
							if (Lb > 1 && Lh == 0)  //неоднозначность по инв. номеру
							{
								MSG('e', "Дубли (" + Lb + ") инвентарного номера!");
								this.btnOK.Hide();
								return;
								...
								if (Lb == 0 && Lh > 1)  //неоднозначность по метке
								{
									MSG('e', "Дубли (" + Lh + ") метки/штрих-кода!");
									this.btnOK.Hide();
									return;
									...
									if (Lb > 1 && Lh > 1)  //неоднозначность полная
									{
										MSG('e', "Дубли по инв. номеру (" + Lb + ") и метке (" + Lh + ")!");
										this.btnOK.Hide();
										return;
